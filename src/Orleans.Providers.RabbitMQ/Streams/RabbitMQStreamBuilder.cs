...
namespace Orleans.Providers.RabbitMQ.Streams
{
    ...
    public class ClusterClientRabbitMQStreamConfigurator<TDataAdapter> : ClusterClientPersistentStreamConfigurator
        where TDataAdapter : IRabbitMQMapper
    {
        public ClusterClientRabbitMQStreamConfigurator(string name, IClientBuilder builder)
            : base(name, builder, RabbitMQAdapterFactory<TDataAdapter>.Create)
        {
            this.clientBuilder.ConfigureApplicationParts(parts => parts.AddFrameworkPart(typeof(RabbitMQAdapterFactory<>).Assembly))
                 .ConfigureServices(services =>
                    services.ConfigureNamedOptionForLogging<RabbitMQStreamProviderOptions>(name)
                    .AddTransient<IConfigurationValidator>(sp => new RabbitMQStreamProviderOptionsValidator(sp.GetOptionsByName<RabbitMQStreamProviderOptions>(name), name))
                    .ConfigureNamedOptionForLogging<HashRingStreamQueueMapperOptions>(name));
        ...
        public ClusterClientRabbitMQStreamConfigurator<TDataAdapter> ConfigureRabbitMQ(Action<OptionsBuilder<RabbitMQStreamProviderOptions>> configureOptions)
        {
            this.Configure<RabbitMQStreamProviderOptions>(configureOptions);
            return this;
        }

        public ClusterClientRabbitMQStreamConfigurator<TDataAdapter> ConfigurePartitioning(int numOfPartition = HashRingStreamQueueMapperOptions.DEFAULT_NUM_QUEUES)
        {
            this.Configure<HashRingStreamQueueMapperOptions>(ob => ob.Configure(options => options.TotalQueueCount = numOfPartition));
            return this;
        }
